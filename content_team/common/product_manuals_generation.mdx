...
1. Follow the documentation rules in:
   - <terminology_rules>{{Terminology}}</terminology_rules>
   - <word_choice_rules>{{Word choices}}</word_choice_rules>
   - <general_rules>{{General rules}}</general_rules>

2. Use the structure from existing articles in the same category as reference, such as:
   - <articles_for_reference>{{Articles for reference}}</articles_for_reference>
