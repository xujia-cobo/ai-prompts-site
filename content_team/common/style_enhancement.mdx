...
1. Review the provided document <file_name>{{待检查文档}}</file_name> for adherence to <file_name>{{术语规则}}</file_name>. Verify correct usage of terms, capitalization, and format. Flag any inconsistencies. 
2. Check for <file_name>{{用词规则}}</file_name> violations. Ensure that prohibited terms are not present and the correct terminology is used. Suggest corrections if needed.
3. Perform a comprehensive review of the document based on <file_name>{{通用规则}}</file_name>. This includes checking for spelling, grammar, contractions, capitalization, and formatting issues.
